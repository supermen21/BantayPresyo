...
//Insert to du$g_region = $_SESSION['userdata']['region'];
    ...
    {
        $g_date = $_POST['DATE_ENCODED'];        
    }else{
        $g_date = "";                                               
    }

    $sql = 'INSERT INTO tbl_bp_prices (`DATE`,MARKET_CODE,COMMODITY_CODE,CATEGORY_CODE,UNIT,HIGH,LOW,PREVAILING,REMARKS,ENCODER,DATE_ENCODED) VALUES ("' .$g_n_date.'","'.utf8_decode(trim($g_market)).'","'.utf8_decode(trim($g_commodity)).'","'.utf8_decode(trim($g_category)).'","'.utf8_decode(trim($g_unit)).'","'.utf8_decode(trim($g_high)).'","'.utf8_decode(trim($g_low)).'","'.utf8_decode(trim($g_prevailing)).'","'.utf8_decode(trim($g_remarks)).'","'.utf8_decode(trim($upload_user)).'","'.utf8_decode(trim($g_date)).'")';
