    ...
    if(isset($_POST['submitFish'])) { 
        ...
        if(!empty($cat)){
            ...
            for($k = 0; $k < count($cat); $k++){
                ...
                echo "<script>window.location.href='add_lowveg.php?PID=$pid&MARKET=$market&DATEE=$date';</script>";
            }
        }
        
    }

    $getCategory = $dbConn->query("SELECT * FROM ref_category WHERE CATEGORY = 'MEAT AND POULTRY' GROUP BY CATEGORY_CODE");
